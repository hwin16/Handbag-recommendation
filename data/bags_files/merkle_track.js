...
(function(merklesearch, undefined){
   try {
      ...
      var sidSentUsingLandingPageUrl = false;
      merklesearch.sendAdSidUsingLandingPageUrl = function( mid, cid, landingPageUrl ){
        try {
           if ( sidSentUsingLandingPageUrl || !mid ) { return false };
        
           var params = _get_attributes_and_set_cookies(mid, cid, landingPageUrl);
           ...
           if (params.mrkgadid){ 
              ...
              sidSentUsingLandingPageUrl = true;
           };

        } catch(e){console.log(e)};
     }

     merklesearch.sendSidUsingLandingPageUrl = function( mid, cid, landingPageUrl ){
        try { 
           if ( sidSentUsingLandingPageUrl || !mid ) { return false };
      ...
      merklesearch.sendOrder  = function( orderObject ){
         try {
            var attr = _prepare_ordergif(orderObject);
      ...
      merklesearch.sendOrderSkipNavigationCheck  = function( orderObject ){
         try {
            var attr = _prepare_ordergif(orderObject, true);
            if ( !attr ) { return undefined };
      ...
      merklesearch.sendPhoneOrder  = function( orderObject ){
         try {
            var attr = _prepare_phoneordergif(orderObject);
            if ( !attr ) { return undefined };
      ...
      merklesearch.microPhoneOrder  = function( orderObject ){
         try {
            var attr = _prepare_phoneordergif(orderObject);
            if ( !attr ) { return undefined };
      ...
      merklesearch.getSidCookie = function(mid){
         ...
         try {
            ...
            paramsObj.rkg_id = _sidAgeTest(paramsObj.rkg_id);
            
            var mrkgsid = cookies.mrkgsid ? _stripTimestamp(cookies.mrkgsid) : _stripTimestamp(paramsObj.rkg_id);
            _setCookieRefreshTTL("mrkgsid", mrkgsid);
            
            var rkglsid = cookies.rkglsid ? _stripTimestamp(cookies.rkglsid) : mrkgsid;
      ...
      merklesearch.getSids = function(mid){
         ...
         try {
            ...
            return ( rkglsid ? "rkglsid=" + encodeURIComponent(rkglsid) : '') + ( mrkgsid ? "&mrkgsid=" + encodeURIComponent(mrkgsid) : '') + "&v=" + version;
         } catch(e){console.log(e)};
      }
      ...
      function _prepare_ordergif(orderObject, skipPageRefreshCheck){
         ...
         var mrkgsid = _stripTimestamp(cookies.mrkgsid);
         _setCookieRefreshTTL("mrkgsid", mrkgsid);
         ...
         for ( var key in orderObject ) {
          ...
          return attr + (rkglsid ? "&rkglsid=" + encodeURIComponent(rkglsid) : '') + (mrkgsid ? "&mrkgsid=" + encodeURIComponent(mrkgsid) : '') + _getHref() + "&v=" + version;
      }

      function _prepare_phoneordergif(orderObject){
         if ( !orderObject || !orderObject.mid ) { return undefined };
      ...
      function _sidAgeTest(param){
         if (param == 0){return undefined;};

         try {
            var sidTs = param.substring(param.indexOf('t-') + 2, param.length) * 1000;
            var now = new Date();

            if (now - sidTs < 1200000){
               return param;
            }
            else{
               return undefined;
      ...
      function _get_attributes_and_set_cookies(mid, cid, landingPageUrl){
         ...
         var mrkgsid;
         if (paramsObj.mrkgsid) {
            mrkgsid = _stripTimestamp(paramsObj.mrkgsid);
         } else if (cookies.mrkgsid) {
            mrkgsid = _stripTimestamp(cookies.mrkgsid);
         } else {
            mrkgsid = _stripTimestamp(paramsObj.rkg_id);
         }
         ...
         var mrkgsidParam = mrkgsid ? "&mrkgsid=" + encodeURIComponent(mrkgsid) : '';
         ...
         var versionParam = "&v=" + version;
          param.params = rkglsidParam + mrkgsidParam + srcParam + cidParam + _getHref() + versionParam;
         return param;
      }
      ...
      function _getCookies(){
         ...
         for (var i=0; i < cookiesArray.length; i++){
            ...
            if ( cookieName == 'rkglsid' || cookieName == 'mrkgsid') {
